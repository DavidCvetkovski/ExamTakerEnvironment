...
export default function TestAnalyticsDashboardPage() {
    const { testId, runId: rawRunId } = useParams<{ testId: string; runId: string }>();
    // The picker uses "combined" as the URL sentinel for the all-runs view; map
    // back to a null runId so cache keys / store calls stay consistent.
    const runId: string | null =
        !rawRunId || rawRunId === 'combined' ? null : rawRunId;
    ...
    useEffect(() => {
        ...
        const timeout = window.setTimeout(() => {
            void runCutScoreScenarios(testId, [cutScore], runId);
        }, 300);

        return () => window.clearTimeout(timeout);
    }, [bundle, cutScore, runCutScoreScenarios, testId, runId]);

    const testTitle = blueprints.find((blueprint) => blueprint.id === testId)?.title ?? 'Analytics Snapshot';
    // Resolve the active run so the eyebrow can describe the cohort lens.
    const activeRun = useMemo(() => {
        const runs = testId ? runsByTestId[testId] : undefined;
        if (!runs) return null;
        if (runId) return runs.find((r) => r.run_id === runId) ?? null;
        return runs.find((r) => r.is_recommended_default) ?? null;
    }, [runsByTestId, testId, runId]);
    const scenario = scenarios[cacheKey]?.[0]
            ...
            <div className="min-h-full bg-shell-bg text-foreground">
                <div className="border-b border-shell-border bg-shell-surface px-6 py-5">
                    <div className="mx-auto max-w-6xl">
                        <BackButton
                            onClick={() => router.push(`/analytics/tests/${testId}`)}
                        ...
                        <div className="mt-4 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <Button
                                    variant="primary"
                                    size="sm"
                                    onClick={() => void recompute(testId, runId)}
                                >
                                    Recompute
                                </Button>
                            </div>
